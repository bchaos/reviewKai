...
reviewController = (@$scope, @$location, @socket,  $mdDialog ) ->
    $scope.toggleClass = ->
...
recommendationController = (@$scope, $mdDialog, @socket, @$location) ->
...
searchController =  (@$scope,  $mdDialog, @socket, @$location) ->
...
dashboardController = (@$scope,  $mdDialog, @socket,$location) ->
    $scope.isLoading=true;
...
peerController =(@$scope,  $mdDialog, @socket,@$location) ->
    $scope.myLibrary=false
    $scope.scoreName='avgscore'
    $scope.isLoading=true;
    searchObject = $location.search();
...
genericController
    .$inject= ['$scope']
libraryController = (@$scope,  $mdDialog, @socket, @$location) ->
    $scope.isOpen = false;
    $scope.demo = {
        isOpen: false,
        count: 0,
...
### angular setup ###
angular
    .module 'reviewApp',['ngAnimate', 'ngRoute','ngResource','ngSanitize', 'ngMaterial'],
    ...
    .directive 'searchcard', ->  
        restrict: 'E',
        templateUrl: 'views/searchcard.html' 
    .directive 'steamtransfer', ->
        restrict: 'E',
	...
	.controller 'confidantController', confidantController
    .controller 'peerController', peerController
